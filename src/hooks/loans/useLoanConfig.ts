...
export const useLoanConfig = () => {
  ...
  const isLoading =
    ...
    loadingOriginationFees

  const error =
    configError ||
    interestConfigsError ||
    originationFeesError
  ...
  const durationRange = useMemo(() => {
    ...
    const minDuration = Math.min(...interestAprConfigs.map(config => Number(config.minDuration)))
    const maxDuration = Math.max(...interestAprConfigs.map(config => Number(config.maxDuration)))
